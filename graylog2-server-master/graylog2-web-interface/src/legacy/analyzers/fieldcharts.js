...
export const FieldChart = {
  ...
  _onFieldHistogramFail($graphElement, opts, error) {
    if (error.additional && error.additional.status === 400) {
      sendFailureEvent(opts.chartid, '字段图表只可用于数字字段.');
    } else {
      const alert = jQuery('<div>').addClass('alert').addClass('alert-warning').text('不能加载字段图表, 请重新加载页面后再次尝试.');
      $graphElement.html(alert);
      const errorMessage = (error.additional ? ` ,原因 ${error.additional.status}` : ` ,错误: ${error.message}`);
      UserNotification.error(`加载字段图表 for '${opts.field}' 失败 ${errorMessage}`);
      console.error(error);
    }
  },
  ...
  updateFieldChartData(graphId, options, seriesName) {
    ...
    promise.then(
      ...
      error => {
        UserNotification.error(`更新字段图表数据失败: ${error}`, '不能更新字段图表数据');
      }
    );
  },

...
const BusinessWebErrorDetailPage = React.createClass({
  ...
  getData(data){
    ...
    var endTo = 25;
    var _this = this;
    function mactchResults(bb,cc,param){
      tempArr = [];
    ...
    if(this.state.isTimeScope){//绝对
      var d1= $("#errorFlowStartTime").find('input').val();
      if(d1 == undefined){
        return;
      }
      ...
      console.log('endDate===',$("#errorFlowEndTime").find('input').val());
              ...
              if(bb.length > 0){
                ...
                for(var i=0; i<bb.length; i++){
                  ...
                  if(bb[i].searchName === "业务ID"){
                    var aa = resultArr;
                    if(!(isOr) && aa.length > 0){
                      mactchResults(bb[i],aa,"businessID");
                    }else{
                      mactchResults(bb[i],cc,"businessID");
                    }
                  }else if(bb[i].searchName === "RA耗时"){
                    var aa = resultArr;
                    if(!(isOr) && aa.length > 0){
                      mactchResults(bb[i],aa,"raDuration");
                    }else{
                      mactchResults(bb[i],cc,"raDuration");
                    }

                  }else if(bb[i].searchName === "CA耗时"){
                    var aa = resultArr;
                    if(!(isOr) && aa.length > 0){
                      mactchResults(bb[i],aa,"caDuration");
                    }else{
                      mactchResults(bb[i],cc,"caDuration");
                    }

                  }else if(bb[i].searchName === "KM耗时"){
                    var aa = resultArr;
                    if(!(isOr) && aa.length > 0){
                      mactchResults(bb[i],aa,"kmDuration");
                    }else{
                      mactchResults(bb[i],cc,"kmDuration");
                    }

                  }else if(bb[i].searchName === "开始时间"){
                    var aa = resultArr;
                    if(!(isOr) && aa.length > 0){
                      mactchResults(bb[i],aa,"startTime");
                    }else{
                      mactchResults(bb[i],cc,"startTime");
                    }

                  }else if(bb[i].searchName === "结束时间"){
                    var aa = resultArr;
                    if(!(isOr) && aa.length > 0){
                      mactchResults(bb[i],aa,"endTime");
                    }else{
                      mactchResults(bb[i],cc,"endTime");
                    }
  ...
  render() {
    ...
    var way = arrayDropValue.map(function(value,i){
      return(
        <li role="presentation" onClick={this.onLiWay.bind(this,i)}><a role="menuitem" tabIndex="-1" href="#">{value}</a></li>
      )
    }.bind(this));
    var allTime = arrayDropTimeValue.map(function(obj,i){
      return(
        <li role="presentation" onClick={this.onLiTime.bind(this,obj)}><a role="menuitem" tabIndex="-1" href="#">{obj.value}</a></li>
      )
    }.bind(this));
    ...
    return (
      <div >
        ...
        <Row className="content">
          <widget.WebErrorDetailModal ref="errorFlowModal"/>
            <div className="form-inline" role="form" style={{textAlign:'start',marginRight:'auto'}}>
              ...
              {
                this.state.isTimeScope ? <div className="form-group" style={{marginLeft:"15px"}}>
                ...
                <div className="form-group" style={{marginLeft:"15px"}}>
                  <div className="form-group">
                      <ButtonToolbar>
                        <DropdownButton title={this.state.dropDownTimeValue} ref="onDropdownValue1"
                                        id='dropdownMenu2' bsStyle="info" onSelect={this.onDropdownSelect}>
                          {allTime}
                        </DropdownButton>
                      </ButtonToolbar>
                  </div>
...
export default BusinessWebErrorDetailPage;

...
const ErrorConditionSearch = React.createClass({
  ...
  getTableData(){
    ...
    var newStr = {"total": selectMult, "str": ""};
    for(var i =0;i< selectMult.length;i++){
      ...
      if(i == selectMult.length-1){
        newStr.str += selectMult[i].searchName+":"+tempCondition+selectMult[i].value;
      }else{
        newStr.str += selectMult[i].searchName+":"+tempCondition+selectMult[i].value+(radioValue == "1" ? " AND ":" OR ");
      }
    }
  ...
  render() {
    ...
    let strIdNames = ["业务ID","RA耗时","CA耗时","KM耗时","开始时间","结束时间"];

    const mySearchNamesHtml = strIdNames.map(function(mySearchNames,m){
    ...
    return (
      <div className="modal fade" id="errorInfoModal" tabIndex="-1" role="dialog" aria-labelledby="infoModalLabel" aria-hidden="true">
...
export default ErrorConditionSearch;

...
const minutesToSeconds = (minutes) => moment.duration(minutes, 'minutes').as('seconds');

const DEFAULT_RANGE_IN_SECS = hoursToSeconds(1);
const SUPPORTED_RANGES_IN_SECS = [minutesToSeconds(5), minutesToSeconds(30), hoursToSeconds(1), daysToSeconds(1), daysToSeconds(7), daysToSeconds(31), daysToSeconds(365), 0];
...
const SourceOverview = React.createClass({
  ...
  _applyRangeFromHash() {
    ...
    this.changeRangeTitle(range);
  },

  changeRangeTitle(newRangeType) {
    if(newRangeType == minutesToSeconds(5)){
      this.setState({rangeTitle: '过去5分钟'});
    }else if(newRangeType == minutesToSeconds(30)){
      this.setState({rangeTitle: '过去30分钟'});
    }else if(newRangeType == hoursToSeconds(1)){
      this.setState({rangeTitle: '过去一小时'});
    }else if(newRangeType == daysToSeconds(1)){
      this.setState({rangeTitle: '过去一天'});
    }else if(newRangeType == daysToSeconds(7)){
      this.setState({rangeTitle: '过去一周'});
    }else if(newRangeType == daysToSeconds(31)){
      this.setState({rangeTitle: '过去一个月'});
    }else if(newRangeType == daysToSeconds(365)){
      this.setState({rangeTitle: '过去一年'});
    }else{
      this.setState({rangeTitle: '全部'});
    }
  },
  ...
  _onRangeChanged(event, newRangeType) {
    this.changeRange(newRangeType, true);
    this.changeRangeTitle(newRangeType);
  },
      ...
      <div className="row content">
        <div className="col-md-12">
          <div className="alert alert-info">
            没有找到这个时间范围的消息来源，请尝试选择一个不同的时间范围&#63;
          </div>
        </div>
      </div>
    );
        ...
        <div className="row content">
          <div className="col-md-12">
            <div>
              ...
              <ButtonToolbar>
                <DropdownButton title={this.state.rangeTitle} ref="rangeSelector"
                                id='rangeSelector' bsStyle="info" onSelect={this._onRangeChanged}>
                  <MenuItem eventKey={minutesToSeconds(5)}>过去5分钟</MenuItem>
                  <MenuItem eventKey={minutesToSeconds(30)}>过去30分钟</MenuItem>
                  <MenuItem eventKey={hoursToSeconds(1)}>过去一小时</MenuItem>
                  <MenuItem eventKey={daysToSeconds(1)}>过去一天</MenuItem>
                  <MenuItem eventKey={daysToSeconds(7)}>过去一周</MenuItem>
                  <MenuItem eventKey={daysToSeconds(31)}>过去一个月</MenuItem>
                  <MenuItem eventKey={daysToSeconds(365)}>过去一年</MenuItem>
            ...
            <p className="description">
              这是发送消息给DeepLOG的所有日志来源(Source)的一个列表。请注意，这个列表是缓存的几秒内的信息，所以你可能需要等待一会直到一个新的来源出现。
            </p>

            <SupportLink>
              {' 使用鼠标来与页面中的表格和图表进行交互，以对发送数据给 DeepLOG 的日志来源获得一个更好的概览。'}
            </SupportLink>
          </div>
        </div>

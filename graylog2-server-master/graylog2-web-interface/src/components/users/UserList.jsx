...
const UserList = React.createClass({
  ...
  _userInfoFormatter(user) {
    ...
    let userBadge = null;
    if (user.read_only) {
      userBadge = <span><i title="系统用户" className="fa fa-lock"/></span>;
    }
    if (user.external) {
      userBadge = <span><i title="LDAP用户" className="fa fa-cloud"/></span>;
    }
    ...
    if (!user.read_only) {
      const deleteAction = (
        <button id="delete-user" type="button" className="btn btn-xs btn-primary" title="删除用户"
      ...
      const editAction = (
        <LinkContainer to={Routes.SYSTEM.USERS.edit(user.username)}>
          <Button bsStyle="info" bsSize="xs" title={'编辑用户 ' + user.username}>
            编辑
          </Button>
        </LinkContainer>
      );
  ...
  render() {
    ...
    const headers = ['', '名称', '用户名', '邮箱地址', '角色', '动作'];

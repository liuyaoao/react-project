...
const SystemJob = React.createClass({
  _onCancel(job) {
    return (e) => {
      e.preventDefault();
      if (window.confirm(`你确定要取消系统工作 "${job.info}"?`)) {
  ...
  render() {
    ...
    const progress = job.percent_complete < 100 ?
      (<ProgressBar bsStyle="info" active now={job.percent_complete}/>) : <span className="label label-success finished">完成!</span>;
    const cancel = job.is_cancelable ?
      (<button type="button" className="btn btn-primary btn-xs pull-right" onClick={this._onCancel(job)}>取消工作</button>) : null;
    ...
    return (
      <div>
        <div style={{ lineHeight: 1.5, marginBottom: 5 }}>
          ...
          - 开始 <LinkToNode nodeId={job.node_id}/>{' '}

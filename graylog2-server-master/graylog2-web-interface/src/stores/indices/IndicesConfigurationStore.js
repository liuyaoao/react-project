...
const IndicesConfigurationStore = Reflux.createStore({
  ...
  loadRotationConfig() {
    ...
    promise.then(
      ...
      error => {
        UserNotification.error(`获取循环配置失败: ${error}`, '无法检索循环配置');
      });
  ...
  loadRotationStrategies() {
    ...
    promise.then(
      ...
      error => {
        UserNotification.error(`获取循环策略失败: ${error}`, '无法检索循环策略');
      });
  ...
  loadRetentionConfig() {
    ...
    promise.then(
      ...
      error => {
        UserNotification.error(`获取保留配置失败: ${error}`, '无法检索保留配置');
      });
  ...
  loadRetentionStrategies() {
    ...
    promise.then(
      ...
      error => {
        UserNotification.error(`获取保留策略失败: ${error}`, '无法检索保留策略');
      });
  ...
  updateRotationConfiguration(data) {
    ...
    promise.then(
      response => {
        this.trigger({activeRotationConfig: response});
        UserNotification.success('指标循环配置更新成功');
      },
      error => {
        UserNotification.error(`指标循环配置更新失败: ${error}`, '无法更新指标循环配置');
      });
  ...
  updateRetentionConfiguration(data) {
    ...
    promise.then(
      response => {
        this.trigger({activeRetentionConfig: response});
        UserNotification.success('指标保留配置更新成功');
      },
      error => {
        UserNotification.error(`指标保留配置更新失败: ${error}`, '无法更新指标保留配置');
      });

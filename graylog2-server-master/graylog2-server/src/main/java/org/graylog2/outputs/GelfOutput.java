...
public class GelfOutput implements MessageOutput {
    ...
    public static class Config extends MessageOutput.Config {
        ...
        public ConfigurationRequest getRequestedConfiguration() {
            ...
            configurationRequest.addField(new TextField(CK_HOSTNAME, "目的主机", "", "这是目的主机的名字", ConfigurationField.Optional.NOT_OPTIONAL));
            configurationRequest.addField(new NumberField(CK_PORT, "目的端口", 12201, "这是目的端口", ConfigurationField.Optional.NOT_OPTIONAL, NumberField.Attribute.IS_PORT_NUMBER));
            configurationRequest.addField(new DropdownField(CK_PROTOCOL, "协议", "TCP", protocols, "这是用来联接的协议", ConfigurationField.Optional.NOT_OPTIONAL));
            configurationRequest.addField(new NumberField(CK_CONNECT_TIMEOUT, "TCP连接超时", 1000, "在TCP连接超时毫秒", ConfigurationField.Optional.OPTIONAL, NumberField.Attribute.ONLY_POSITIVE));
            configurationRequest.addField(new NumberField(CK_RECONNECT_DELAY, "TCP连接延时", 500, "连接之间的等待时间在毫秒", ConfigurationField.Optional.OPTIONAL, NumberField.Attribute.ONLY_POSITIVE));
            configurationRequest.addField(new BooleanField(CK_TCP_NO_DELAY, "TCP没有延迟", false, "是否为TCP连接使用Nagle算法"));
            configurationRequest.addField(new BooleanField(CK_TCP_KEEP_ALIVE, "TCP保持活动", false, "是否发送TCP保持活动包"));
            configurationRequest.addField(new BooleanField(CK_TLS_VERIFICATION_ENABLED, "TLS验证", false, "是否验证时使用TLS同行"));
            configurationRequest.addField(new TextField(CK_TLS_TRUST_CERT_CHAIN, "TLS证书信任链", "", "包含信任证书链的本地文件", ConfigurationField.Optional.OPTIONAL));
